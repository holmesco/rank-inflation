...
struct RankReductionParams {
  // Target rank, supercedes other conditionals (ignored if -1)
  int targ_rank = 1;
  // Tolerance on null space singular value. If no null space and target rank
  // not set, exit.
...
// Retrieves the linear operator of the constraints in the reduced optimal
// solution space. This function is currently specialized to the max clique
// problem.
...
Lemon, Alex, Anthony Man-Cho So, and Yinyu Ye. "Low-rank semidefinite
programming: Theory and applications." Foundations and Trends in
Optimization 2.1-2 (2016): 1-156.
*/
Matrix rank_reduction(const std::vector<SpMatrix>& As, const Matrix& V_init,
                      RankReductionParams params = RankReductionParams());
}  // namespace RankTools